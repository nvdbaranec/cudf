                         ...
                         rmm::cuda_stream_view stream)
{
  ...
  auto pc_iter = cudf::detail::make_counting_transform_iterator(0, cuda::proclaim_return_type<size_type>([num_page_counts, page_counts = page_counts.begin()] __device__ (size_type i){
    return i >= num_page_counts ? 0 : page_counts[i];
  }));
  thrust::exclusive_scan(rmm::exec_policy_nosync(stream),
                         pc_iter,
                         pc_iter + num_page_counts + 1,
